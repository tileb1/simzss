...
def get_coco(args):
    ...
    dataset = COCOSegmentationEZMapped80(args.patch_size,
...
def group_by_keys_nothrow(data, keys=base_plus_ext, lcase=True, suffixes=None, handler=None):
    ...
    for filesample in data:
        ...
        # This drops only about 0.05% of samples on mscoco
...
def get_wds_dataset(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    def filter_spacy(noun_to_index, tokenizer_decoder, sample):
        ...
        all_nouns = []
        tensor_nouns_indices = torch.zeros_like(sample["tensor_nouns_indices.pyd"], dtype=torch.long)
        tensor_concept_indices = torch.zeros_like(sample["tensor_concept_indices.pyd"], dtype=torch.long)
        ...
        for concept, noun, indices_row, indices_row2 in zip(sample["all_concepts.pyd"], sample["all_nouns.pyd"],
    ...
    noun_to_index = {noun: i for i, noun in enumerate(list(sorted(list(set(valid_nouns)))))}

    pipeline.extend([
        wds.select(filter_no_caption_or_no_image),
        wds.decode("pilrgb", handler=log_and_continue),
        wds.rename(image="jpg;png;jpeg;webp", text="txt"),
        wds.map_dict(image=preprocess_img, text=lambda text: [tokenizer(text)[0], text], __url__=get_shard_id),
        wds.map(partial(filter_spacy, noun_to_index, tokenizer_decoder)),
        wds.to_tuple("__url__", "image", "text", "tensor_nouns_indices", "tensor_concept_indices", "all_nouns",
                     "all_nounsID_tensor"),
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None):
    ...
    if args.coco_val is not None and args.coco_val != "":
